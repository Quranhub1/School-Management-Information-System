...
export default function AcademicYearsPage() {
  ...
  async function load() {
    setLoading(true);
    try {
      const response = await fetch('/api/academic-structure/years', { headers: { Accept: 'application/json', ...(sessionStorage.getItem('smis.accessToken') ? { Authorization: `Bearer ${sessionStorage.getItem('smis.accessToken')}` } : {}) } });
  ...
  async function createYear(event: React.FormEvent) {
    ...
    try {
      const response = await fetch('/api/academic-structure/years', {
        method: 'POST', headers: { 'Content-Type': 'application/json', Accept: 'application/json', ...(sessionStorage.getItem('smis.accessToken') ? { Authorization: `Bearer ${sessionStorage.getItem('smis.accessToken')}` } : {}) },
