...
export const getCurriculumCourses = (id:string) => request<CurriculumCourse[]>(`/api/curriculum-management/curricula/${id}/courses`)
export const addCurriculumCourse = (id:string,input:{courseId:string;yearOfStudy:number;semesterNumber:number;isCore:boolean}) => request<CurriculumCourse>(`/api/curriculum-management/curricula/${id}/courses`,{method:'POST',body:JSON.stringify(input)})
