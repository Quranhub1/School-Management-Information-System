...
export function listPayroll(staffMemberId?:string){ return request<PayrollRecord[]>(`/api/accounts-overview/payroll${staffMemberId ? `?staffMemberId=${encodeURIComponent(staffMemberId)}` : ''}`); }
...
export function markPayrollPaid(id:string, method?: string){ return request<void>(`/api/staff/payroll/${id}/pay`,{method:'PATCH',body:JSON.stringify({paymentMethod: method})}); }
