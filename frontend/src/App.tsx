...
const modules:{key:ModuleKey;label:string}[]=[
  ...
  ['laboratories','Laboratories & Welfare'],
  ['inventory','Inventory & Property'],
  ['communication','Communication'],
  ['guild','Guild'],
  ['reports','Reports & Analytics'],
  ['system-administration','System Administration'],
].map(([key,label])=>({key:key as ModuleKey,label})); const [rptTab,setRptTab]=useState<'cards'|'receipts'|'certificates'|'analytics'>('cards'); const visibleModules=modules.filter(m=>{
  if(m.key==='dashboard') return true;
  if(m.key==='administration') return a;
  if(m.key==='students') return st || ad || s360;
  if(m.key==='academics') return ac || lecturer;
  if(m.key==='finance') return fi;
  if(m.key==='staff') return sr;
  if(m.key==='attendance') return att;
  if(m.key==='library') return lr;
  if(m.key==='health') return att || r.includes('Nurse') || r.includes('ClinicalInstructor') || r.includes('SystemAdministrator');
  if(m.key==='laboratories') return inv;
  if(m.key==='inventory') return inv;
  if(m.key==='communication') return cm;
  if(m.key==='guild') return st || r.includes('Guild') || r.includes('SystemAdministrator');
  if(m.key==='reports') return rp || analytics;
  if(m.key==='system-administration') return a;
  return false
}); return <main className="app-shell"><aside className="sidebar"><div className="sidebar-brand">{institution.logoPath&&<img src={institution.logoPath} alt="" className="sidebar-logo" />}<div><span className="sidebar-kicker">{institution.abbreviation?.trim()||institution.institutionName}</span><h1>{institution.institutionName}</h1>{institution.motto&&<p>{institution.motto}</p>}</div></div><nav className="sidebar-nav">
  <div className="sidebar-nav-section">
    <h2 className="sidebar-section-heading">School Management</h2>
    {visibleModules.map(m=>{const Icon=iconByModule[m.key];return <button key={m.key} className={`sidebar-item ${activeModule===m.key?'active':''}`} onClick={()=>setActiveModule(m.key)} aria-current={activeModule===m.key?'page':undefined}><Icon size={18} strokeWidth={activeModule===m.key?2.2:1.8}/><span>{m.label.toUpperCase()}</span></button>})}
  </div>
