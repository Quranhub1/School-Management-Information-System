...
import { canManageAcademics, canManageExaminations, canManageFinance, canManageStudents } from './auth/roleGuards'
...
function AuthenticatedWorkspace({ onLogout }: { onLogout: () => void }) {
  ...
  const academicAccess = canManageAcademics(roles); const examinationAccess = canManageExaminations(roles); const studentAccess = canManageStudents(roles); const financeAccess = canManageFinance(roles)
  function signOut() { logout(); onLogout() }
  return <main className="app-shell"><header className="topbar"><div><span className="eyebrow">SMIS</span><h1>Institutional Services</h1></div><div className="topbar-actions"><span className="status">{session?.username ?? 'Authenticated'}</span><button className="secondary-button" onClick={signOut}>Sign out</button></div></header><RoleNavigation roles={roles}/>{studentAccess && <StudentManagement canManage/>}{academicAccess && <AcademicManagement canManage/>}{examinationAccess && <ExaminationResults/>}{financeAccess && <FinanceManagement/>}{!studentAccess && !academicAccess && !examinationAccess && !financeAccess && <section className="panel"><h3>Institutional Services</h3><p className="empty">Your role has no management workspace assigned yet.</p></section>}</main>
}
