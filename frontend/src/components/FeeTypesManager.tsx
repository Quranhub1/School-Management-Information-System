...
import { applyFeeStructure, createFeeStructure, getFeeStructures, type FeeStructure } from '../api/finance'
...
type ItemDraft = { code: string; name: string; amount: string }
...
export function FeeTypesManager() {
  ...
  const [items, setItems] = useState<ItemDraft[]>([{ code: 'FEE', name: '', amount: '' }])
  ...
  function addItem() { setItems(current=>[...current,{code:'FEE-' + (current.length+1),name:'',amount:''}]) }
  ...
  async function createAndMaybeApply() {
    ...
    try {
      const structure=await createFeeStructure({
        ...
        items:items.map((x,i)=>({code:x.code.trim(),name:x.name.trim(),amount:Number(x.amount),sortOrder:i,isOptional:false}))
      })
      let result=''
      ...
      setName(''); setFeeType(''); setItems([{code:'FEE',name:'',amount:''}]); await load()
  ...
  return <section className="fee-types-manager">
    ...
    <div className="fee-builder">
      ...
      <div className="fee-items-list"><div className="fee-items-heading"><strong>Fee items</strong><span>Total is calculated from the items, because apparently arithmetic deserves governance.</span></div>{items.map((item,i)=><div className="fee-item-row" key={i}><input value={item.code} onChange={e=>updateItem(i,'code',e.target.value)} placeholder="BUS" /><input value={item.name} onChange={e=>updateItem(i,'name',e.target.value)} placeholder="School Bus / Transport" /><input type="number" min="0.01" step="0.01" value={item.amount} onChange={e=>updateItem(i,'amount',e.target.value)} placeholder="150000" />{items.length>1&&<button type="button" className="secondary-button" onClick={()=>setItems(items.filter((_,x)=>x!==i))}>Remove</button>}</div>)}<button type="button" className="secondary-button" onClick={addItem}>+ Add fee item</button></div>
    ...
    <div className="panel" style={{marginTop:22}}><div className="panel-heading"><div><p className="eyebrow">ACTIVE STRUCTURES</p><h3>Configured fee types</h3></div><button className="secondary-button" onClick={()=>void load()}>Refresh</button></div><div className="table-wrap"><table><thead><tr><th>Fee</th><th>Type</th><th>Items</th><th>Total</th><th>Status</th><th /></tr></thead><tbody>{structures.map(s=><tr key={s.id}><td><strong>{s.name}</strong></td><td>{s.feeType}</td><td>{s.items.map(i=>i.name).join(', ')}</td><td>{formatCurrency(s.totalAmount,s.currency||SYSTEM_CURRENCY)}</td><td>{s.isActive?'Active':'Inactive'}</td><td><button className="secondary-button" onClick={()=>void apply(s.id,s.name)}>Apply to Active Students</button></td></tr>)}{structures.length===0&&<tr><td colSpan={6}>No fee structures configured yet.</td></tr>}</tbody></table></div></div>
  </section>
}
