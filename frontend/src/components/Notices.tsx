...
export function Notices({ canManage }: { canManage?: boolean }) {
  ...
  async function submit(e: React.FormEvent) { e.preventDefault(); setError(''); try { await createNotice({ ...form, createdBy: '', createdAt: '' } as any); setForm({ title: '', body: '', priority: 'General', audience: 'All', expiresAt: '' }); await load() } catch (e) { setError(e instanceof Error ? e.message : 'Unable to post notice.') } }
  ...
  return (
    <section className="panel" aria-label="Notices">
      ...
      {loading ? <p className="empty">Loading notices…</p> : <div className="table-wrap"><table><thead><tr><th>Title</th><th>Priority</th><th>Audience</th></tr></thead><tbody>{notices.map(n => <tr key={n.id}><td>{n.title}</td><td>{n.priority}</td><td>{n.audience}</td></tr>)}</tbody></table></div>}
    </section>
  );
}
