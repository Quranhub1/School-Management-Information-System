...
export function TimetableManagement() {
  ...
  async function submit(event: React.FormEvent) {
    ...
    setLoading(true)
    try {
      await createTimetableEntry({ teachingGroupId: teachingGroupId.trim(), staffMemberId: staffMemberId || undefined, dayOfWeek: Number(dayOfWeek), startTime: `${startTime}:00`, endTime: `${endTime}:00`, room: room.trim() || undefined, sessionType: sessionType.trim() || undefined })
      await load()
  ...
  return <section className="panel"><div className="section-heading"><div><p className="eyebrow">Scheduling</p><h2>Timetable Management</h2></div></div>
    <form className="management-form" onSubmit={submit}><label>Teaching group ID<input value={teachingGroupId} onChange={e => setTeachingGroupId(e.target.value)} /></label><label>Staff<select value={staffMemberId} onChange={e => setStaffMemberId(e.target.value)}><option value="">— None —</option>{staff.map(s => <option key={s.id} value={s.id}>{s.firstName} {s.lastName} ({s.staffNumber})</option>)}</select></label><label>Day<select value={dayOfWeek} onChange={e => setDayOfWeek(e.target.value)}>{days.map((d, i) => <option key={d} value={i}>{d}</option>)}</select></label><label>Start<input type="time" value={startTime} onChange={e => setStartTime(e.target.value)} /></label><label>End<input type="time" value={endTime} onChange={e => setEndTime(e.target.value)} /></label><label>Room<input value={room} onChange={e => setRoom(e.target.value)} /></label><label>Session type<input value={sessionType} onChange={e => setSessionType(e.target.value)} /></label><button type="submit" disabled={loading}>{loading ? 'Saving…' : 'Schedule session'}</button></form>
    {error && <div className="error" role="alert">{error}</div>}
    <div className="table-wrap"><table><thead><tr><th>Day</th><th>Time</th><th>Room</th><th>Type</th><th>Group</th><th>Staff</th><th /></tr></thead><tbody>{entries.map(entry => <tr key={entry.id}><td>{days[entry.dayOfWeek]}</td><td>{entry.startTime.slice(0,5)}–{entry.endTime.slice(0,5)}</td><td>{entry.room ?? '—'}</td><td>{entry.sessionType ?? '—'}</td><td>{entry.teachingGroupId}</td><td>{entry.staffMemberId ?? '—'}</td><td><button className="secondary-button" onClick={() => void remove(entry.id)}>Remove</button></td></tr>)}{entries.length === 0 && <tr><td colSpan={7} className="empty">No timetable sessions scheduled.</td></tr>}</tbody></table></div>
  </section>
}
