          ...
          <tbody>
            {admissions.map(a => (
              <tr key={a.id}>
                ...
                <td>{a.status}</td>
                <td>
                  <select value={selectedId === a.id ? updateStatus : a.status} onChange={e => { setSelectedId(a.id); setUpdateStatus(e.target.value); changeStatus(a.id, e.target.value) }}>{statuses.map(s => <option key={s} value={s}>{s}</option>)}</select>
