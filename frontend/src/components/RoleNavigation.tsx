...
export function RoleNavigation({ roles }: RoleNavigationProps) {
  const items = [hasAnyRole(roles, ADMINISTRATION_ROLES) ? 'Administration' : null, hasAnyRole(roles, ACADEMIC_MANAGEMENT_ROLES) ? 'Academic Management' : null, hasAnyRole(roles, EXAMINATION_MANAGEMENT_ROLES) ? 'Examinations' : null, hasAnyRole(roles, STUDENT_MANAGEMENT_ROLES) ? 'Student Management' : null, hasAnyRole(roles, FINANCE_MANAGEMENT_ROLES) ? 'Finance' : null, roles.includes('Lecturer') ? 'Teaching' : null, roles.includes('Student') ? 'Student Portal' : null].filter(Boolean) as string[]
