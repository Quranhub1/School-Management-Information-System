...
public sealed class BulkImportService(IStudentRepository students, IHrRepository staff)
{
    public async Task<BulkImportResult> ImportStudentsAsync(BulkImportRequest request, CancellationToken cancellationToken = default)
    {
        ...
        for (var i = 1; i < lines.Length; i++)
        {
            ...
            if (string.IsNullOrWhiteSpace(studentNumber) || string.IsNullOrWhiteSpace(firstName) || string.IsNullOrWhiteSpace(lastName))
            {
                errors.Add($"Row {i + 1}: StudentNumber, FirstName and LastName are required.");
                continue;
            }

            if (existingNumbers.Contains(studentNumber))
            {
                warnings.Add($"Row {i + 1}: Student with number {studentNumber} already exists.");
                continue;
            }
    ...
    public async Task<BulkImportResult> ImportStaffAsync(BulkImportRequest request, CancellationToken cancellationToken = default)
    {
        ...
        if (staffNumberIdx < 0 || firstNameIdx < 0 || lastNameIdx < 0 || employmentTypeIdx < 0)
            return new BulkImportResult(0, 1, new[] { "CSV must contain: StaffNumber, FirstName, LastName, EmploymentType." }, Array.Empty<string>());
        ...
        for (var i = 1; i < lines.Length; i++)
        {
            ...
            if (string.IsNullOrWhiteSpace(staffNumber) || string.IsNullOrWhiteSpace(firstName) || string.IsNullOrWhiteSpace(lastName) || string.IsNullOrWhiteSpace(employmentType))
            {
                errors.Add($"Row {i + 1}: StaffNumber, FirstName, LastName and EmploymentType are required.");
                continue;
            }

            if (existingNumbers.Contains(staffNumber))
            {
                warnings.Add($"Row {i + 1}: Staff with number {staffNumber} already exists.");
                continue;
            }
    ...
    public async Task<string> ExportStudentsAsync(IReadOnlyDictionary<string, string?>? filters, CancellationToken cancellationToken = default)
    {
        ...
        if (filters != null)
        {
            ...
            if (filters.TryGetValue("search", out var search) && !string.IsNullOrWhiteSpace(search))
                query = query.Where(s => s.StudentNumber.Contains(search.Trim()) || s.FirstName.Contains(search.Trim()) || s.LastName.Contains(search.Trim()));
        }

        var students = query.OrderBy(s => s.LastName).ThenBy(s => s.FirstName).ToList();

        var sb = new StringBuilder();
        sb.AppendLine("StudentNumber,FirstName,LastName,OtherNames,DateOfBirth,Gender,PhoneNumber,Email,Status,CreatedAt");
        foreach (var s in students)
        {
            sb.AppendLine($"{Escape(s.StudentNumber)},{Escape(s.FirstName)},{Escape(s.LastName)},{Escape(s.OtherNames)},{Escape(s.DateOfBirth?.ToString("yyyy-MM-dd"))},{Escape(s.Gender)},{Escape(s.PhoneNumber)},{Escape(s.Email)},{Escape(s.Status)},{Escape(s.CreatedAt.ToString("o"))}");
        }
    ...
    public async Task<string> ExportStaffAsync(IReadOnlyDictionary<string, string?>? filters, CancellationToken cancellationToken = default)
    {
        ...
        if (filters != null)
        {
            if (filters.TryGetValue("activeOnly", out var activeOnlyStr) && bool.TryParse(activeOnlyStr, out var activeOnly) && activeOnly)
                query = query.Where(s => s.IsActive);
            if (filters.TryGetValue("search", out var search) && !string.IsNullOrWhiteSpace(search))
                query = query.Where(s => s.StaffNumber.Contains(search.Trim()) || s.FirstName.Contains(search.Trim()) || s.LastName.Contains(search.Trim()));
        }
        ...
        {
            sb.AppendLine($"{Escape(s.StaffNumber)},{Escape(s.FirstName)},{Escape(s.LastName)},{Escape(s.NationalId)},{Escape(s.PhoneNumber)},{Escape(s.Email)},{Escape(s.EmploymentType)},{s.IsActive}");
        }
