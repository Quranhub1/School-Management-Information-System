...
public sealed class ReportGenerator(SchoolManagementDbContext db) : IReportService
{
    public ReportDto BuildSummary(ReportMetrics metrics) => new ReportDto("summary", metrics.Students, new Dictionary<string, long> { ["students"] = metrics.Students, ["staff"] = metrics.Staff, ["programmes"] = metrics.Programmes, ["courses"] = metrics.Courses, ["academicYears"] = metrics.AcademicYears, ["currentAcademicYears"] = metrics.CurrentAcademicYears, ["activeLibraryBooks"] = metrics.ActiveLibraryBooks, ["activeLibraryLoans"] = metrics.ActiveLibraryLoans }, new Dictionary<string, decimal>());

    public ReportDto BuildFinanceSummary(FinanceReportMetrics metrics) => new ReportDto("finance", metrics.InvoiceCount, new Dictionary<string, long> { ["invoiceCount"] = metrics.InvoiceCount, ["paymentCount"] = metrics.PaymentCount }, new Dictionary<string, decimal> { ["totalInvoiced"] = metrics.TotalInvoiced, ["totalPaid"] = metrics.TotalPaid, ["outstanding"] = metrics.TotalInvoiced - metrics.TotalPaid });

    public ReportDto BuildAttendanceSummary(AttendanceReportMetrics metrics) => new ReportDto("attendance", metrics.Sessions, new Dictionary<string, long> { ["sessions"] = metrics.Sessions, ["studentAttendanceRecords"] = metrics.StudentAttendanceRecords, ["attendanceRecords"] = metrics.AttendanceRecords }, new Dictionary<string, decimal>());

    public ReportDto BuildAcademicSummary(AcademicReportMetrics metrics) => new ReportDto("academic", metrics.AcademicYears, new Dictionary<string, long> { ["academicYears"] = metrics.AcademicYears, ["semesters"] = metrics.Semesters, ["programmes"] = metrics.Programmes, ["courses"] = metrics.Courses, ["registrations"] = metrics.Registrations, ["results"] = metrics.Results }, new Dictionary<string, decimal>());
    ...
    public async Task<StudentReportCardDto?> GenerateStudentReportCardAsync(Guid studentId, Guid? academicYearId, Guid? semesterId, CancellationToken cancellationToken = default)
    {
        ...
        var formatted = results.Select(r => new StudentReportCardDto.ResultEntry(
            r.CourseCode,
            r.CourseName,
            r.Score,
            r.Grade,
    ...
    public async Task<FeeReceiptDto?> GenerateFeeReceiptAsync(Guid paymentId, CancellationToken cancellationToken = default)
    {
        ...
        return new FeeReceiptDto(
            ...
            payment.StudentInvoice!.InvoiceNumber,
            payment.StudentInvoice.StudentId,
    ...
    public async Task<AttendanceReportDto> GenerateAttendanceReportAsync(Guid classId, (DateOnly From, DateOnly To) dateRange, CancellationToken cancellationToken = default)
    {
        var sessions = await db.AttendanceSessions.AsNoTracking()
            ...
            .Where(s => s.TimetableEntry.TeachingGroupId == classId && s.SessionDate >= dateRange.From && s.SessionDate <= dateRange.To)
        ...
        return new AttendanceReportDto(
            ...
            records.Count,
            summary,
            sessions.Select(s => new AttendanceReportDto.SessionSummary(s.Id, s.SessionDate, s.Status, records.Count(r => r.AttendanceSessionId == s.Id))).ToList());
    }
