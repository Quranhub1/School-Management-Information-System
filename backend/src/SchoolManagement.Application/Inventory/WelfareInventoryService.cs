...
public sealed class WelfareInventoryService(SchoolManagementDbContext db)
{
    ...
    {
        var commodity = await db.WelfareCommodities.AsNoTracking().SingleOrDefaultAsync(x => x.Id == commodityId, ct)
            ?? throw new KeyNotFoundException("Welfare commodity was not found.");
    ...
    public async Task<TransactionDto> RecordAsync(Guid commodityId, DateOnly date, string type, decimal quantity, string? supplier, string? reference, string? batchNumber, DateOnly? expiryDate, string? notes, string recordedBy, CancellationToken ct)
    {
        ...
        if (!string.Equals(type, "Adjustment", StringComparison.OrdinalIgnoreCase) && quantity < 0) throw new ArgumentException("Quantity must be positive for receipts, consumption and wastage.");
