...
public sealed class BankReconciliationService(IBankReconciliationRepository repository, IFinanceRepository finance)
{
    ...
    public Task<IReadOnlyList<BankStatementLine>> GetLinesAsync(Guid reconciliationId, CancellationToken cancellationToken = default) =>
        repository.GetLinesAsync(reconciliationId, cancellationToken);
