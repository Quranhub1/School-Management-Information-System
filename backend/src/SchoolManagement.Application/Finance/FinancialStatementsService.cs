...
public sealed record BudgetVsActualRow(Guid AccountId, string AccountCode, string AccountName, decimal Budget, decimal Actual, decimal Variance);
...
public sealed class FinancialStatementsService(IFinanceRepository finance)
{
    public async Task<IncomeStatement> GetIncomeStatementAsync(DateOnly? from = null, DateOnly? to = null, CancellationToken cancellationToken = default)
    {
        ...
        var rows = lines.GroupBy(x => x.Account.Id).Select(g => ToRow(g.Key, g.Select(x => x.Account).First(), g.Sum(x => x.Debit - x.Credit))).OrderBy(x => x.AccountCode).ToList();
    ...
    public async Task<BalanceSheet> GetBalanceSheetAsync(DateOnly? to = null, CancellationToken cancellationToken = default)
    {
        ...
        var rows = Flatten(entries).Where(x => IsType(x.Account.AccountType, "Asset", "Assets", "Liability", "Liabilities", "Equity")).GroupBy(x => x.Account.Id)
            .Select(g => ToRow(g.Key, g.Select(x => x.Account).First(), g.Sum(x => x.Debit - x.Credit))).OrderBy(x => x.AccountCode).ToList();
    ...
    private static FinancialStatementRow ToRow(Guid _, Account account, decimal signedBalance) => new(account.Code, account.Name, account.AccountType, signedBalance);
