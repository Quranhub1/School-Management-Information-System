...
public sealed class SchoolManagementDbContext(DbContextOptions<SchoolManagementDbContext> options) : DbContext(options)
{
    ...
    protected override void OnModelCreating(ModelBuilder m)
    {
        ...
        m.Entity<AttendanceSession>(e =>
        {
            e.HasKey(x => x.Id);
            e.HasIndex(x => new { x.TimetableEntryId, x.SessionDate }).IsUnique();
            e.Property(x => x.Status).HasMaxLength(30).IsRequired();
            e.Property(x => x.Remarks).HasMaxLength(500);
            e.HasOne<TimetableEntry>().WithMany().HasForeignKey(x => x.TimetableEntryId).OnDelete(DeleteBehavior.Restrict);
        });
        m.Entity<StudentAttendance>(e =>
        {
            e.HasKey(x => x.Id);
            e.HasIndex(x => new { x.AttendanceSessionId, x.StudentId }).IsUnique();
            e.Property(x => x.Status).HasMaxLength(30).IsRequired();
            e.Property(x => x.Remarks).HasMaxLength(500);
            e.HasOne<AttendanceSession>().WithMany().HasForeignKey(x => x.AttendanceSessionId).OnDelete(DeleteBehavior.Cascade);
