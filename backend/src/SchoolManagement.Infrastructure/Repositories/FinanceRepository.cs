...
public sealed class FinanceRepository(SchoolManagementDbContext db) : IFinanceRepository, IFinanceAdjustmentsRepository, SchoolManagement.Application.Finance.IFinanceRepository
{
    ...
    public Task<CreditNote?> GetCreditNoteForUpdateAsync(Guid creditNoteId, CancellationToken cancellationToken) => db.CreditNotes.FirstOrDefaultAsync(x => x.Id == creditNoteId, cancellationToken);
