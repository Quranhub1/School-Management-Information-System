...
public sealed class FinanceRepository(SchoolManagementDbContext db) : IFinanceRepository, IFinanceAdjustmentsRepository, SchoolManagement.Application.Finance.IFinanceRepository
{
    ...
    public async Task<IReadOnlyList<StudentInvoice>> GetStudentInvoicesAsync(Guid studentId, CancellationToken cancellationToken) => await db.StudentInvoices.AsNoTracking().Include(x => x.Lines).Include(x => x.Discounts).Include(x => x.Installments).Where(x => x.StudentId == studentId).OrderByDescending(x => x.IssuedAt).ToListAsync(cancellationToken);
    public async Task<IReadOnlyList<StudentInvoice>> GetAllStudentInvoicesAsync(CancellationToken cancellationToken) => await db.StudentInvoices.AsNoTracking().Include(x => x.Lines).Include(x => x.Discounts).Include(x => x.Installments).OrderBy(x => x.StudentId).ThenBy(x => x.IssuedAt).ToListAsync(cancellationToken);
    public Task<FeeStructure?> GetActiveFeeStructureAsync(Guid feeStructureId, CancellationToken cancellationToken) => db.FeeStructures.AsNoTracking().Include(x => x.Items.OrderBy(i => i.SortOrder)).FirstOrDefaultAsync(x => x.Id == feeStructureId && x.IsActive, cancellationToken);
    ...
    public Task<Payment?> GetPaymentAsync(Guid paymentId, CancellationToken cancellationToken) => db.Payments.Include(x => x.Allocations).FirstOrDefaultAsync(x => x.Id == paymentId, cancellationToken);
    public async Task<IReadOnlyList<StudentInvoice>> GetOutstandingInvoicesAsync(Guid studentId, string currency, CancellationToken cancellationToken) => await db.StudentInvoices.AsNoTracking().Where(x => x.StudentId == studentId && x.Currency == currency && x.PaidAmount < x.NetAmount).OrderBy(x => x.IssuedAt).ThenBy(x => x.InvoiceNumber).ToListAsync(cancellationToken);
    public async Task<IReadOnlyList<PaymentLedgerEntry>> GetStudentLedgerAsync(Guid studentId, CancellationToken cancellationToken) => await db.PaymentLedgerEntries.AsNoTracking().Where(x => x.StudentId == studentId).OrderBy(x => x.EntryDate).ThenBy(x => x.Id).ToListAsync(cancellationToken);
    public Task<InvoiceDiscount?> GetInvoiceDiscountAsync(Guid discountId, CancellationToken cancellationToken) => db.Set<InvoiceDiscount>().FirstOrDefaultAsync(x => x.Id == discountId, cancellationToken);
    ...
    public Task<bool> CreditNoteNumberExistsAsync(string creditNoteNumber, CancellationToken cancellationToken) => db.CreditNotes.AsNoTracking().AnyAsync(x => x.CreditNoteNumber == creditNoteNumber, cancellationToken);
    ...
    public Task<Refund?> GetRefundAsync(Guid refundId, CancellationToken cancellationToken) => db.Set<Refund>().AsNoTracking().FirstOrDefaultAsync(x => x.Id == refundId, cancellationToken);
    public async Task<IReadOnlyList<Refund>> GetRefundsAsync(Guid paymentId, CancellationToken cancellationToken) => await db.Set<Refund>().AsNoTracking().Where(x => x.PaymentId == paymentId).OrderBy(x => x.RefundedAt).ToListAsync(cancellationToken);
    public Task<bool> RefundNumberExistsAsync(string refundNumber, CancellationToken cancellationToken) => db.Set<Refund>().AsNoTracking().AnyAsync(x => x.RefundNumber == refundNumber, cancellationToken);
    public Task<decimal> GetRefundedAmountAsync(Guid paymentId, CancellationToken cancellationToken) => db.Set<Refund>().AsNoTracking().Where(x => x.PaymentId == paymentId && x.Status == "Completed").SumAsync(x => (decimal?)x.Amount, cancellationToken).ContinueWith(x => x.Result ?? 0m, cancellationToken);
    public async Task AddRefundAsync(Refund refund, CancellationToken cancellationToken) => await db.Set<Refund>().AddAsync(refund, cancellationToken);
