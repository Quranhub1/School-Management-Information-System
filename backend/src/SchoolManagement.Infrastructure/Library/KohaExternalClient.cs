...
public sealed class KohaExternalClient : ILibraryExternalClient
{
    ...
    public KohaExternalClient(ILibraryIntegrationSettings settings, HttpClient httpClient)
    {
    ...
    {
        if (!IsConfigured)
            return new(SystemName, false, false, "KOHA endpoint is not configured.");

        try
        {
            using var request = new HttpRequestMessage(HttpMethod.Head, BaseUri);
            using var response = await _httpClient.SendAsync(request, HttpCompletionOption.ResponseHeadersRead, cancellationToken);
            return new(SystemName, true, true, $"KOHA responded with HTTP {(int)response.StatusCode}.");
        }
        catch (OperationCanceledException) when (!cancellationToken.IsCancellationRequested)
        {
            return new(SystemName, true, false, "KOHA connection timed out.");
        }
        catch (HttpRequestException ex)
        {
            return new(SystemName, true, false, $"KOHA connection failed: {ex.Message}");
        }
    }
}
