...
public sealed class AdmissionsController(AdmissionsWorkflowService workflow, SchoolManagementDbContext db) : ControllerBase
{
    ...
    public async Task<IActionResult> Admit(Guid id, AdmitApplicantRequest request, CancellationToken cancellationToken)
    {
        ...
        if (request.AcademicYearId.HasValue && !await db.AcademicYears.AnyAsync(x => x.Id == request.AcademicYearId.Value, cancellationToken)) return BadRequest(new { message = "The selected academic year does not exist." });
        ...
        if (await db.Students.AnyAsync(x => x.StudentNumber == request.StudentNumber, cancellationToken)) return Conflict(new { message = "The student number is already in use." });

        var student = new Student { StudentNumber = request.StudentNumber.Trim(), FirstName = applicant.FirstName, LastName = applicant.LastName, OtherNames = applicant.OtherNames, DateOfBirth = applicant.DateOfBirth, Gender = applicant.Gender, NationalId = applicant.NationalId, PhoneNumber = applicant.PhoneNumber, Email = applicant.Email, AdmissionId = applicant.Id, Status = "Active" };
        var admission = new Admission { ApplicantId = applicant.Id, StudentId = student.Id, ProgrammeId = request.ProgrammeId, IntakeId = request.IntakeId, AcademicYearId = request.AcademicYearId, AdmissionNumber = request.AdmissionNumber.Trim(), AdmissionType = request.AdmissionType.Trim(), Status = "Admitted", AdmittedAt = DateTimeOffset.UtcNow, ReportingDate = request.ReportingDate, DecisionReference = request.DecisionReference?.Trim() };
        var enrollment = new StudentEnrollment { StudentId = student.Id, ProgrammeId = request.ProgrammeId, IntakeId = request.IntakeId, AdmissionDate = request.ReportingDate ?? DateOnly.FromDateTime(DateTime.UtcNow), Status = "Active", CurrentYear = 1 };
        db.Students.Add(student); db.Admissions.Add(admission); db.StudentEnrollments.Add(enrollment); applicant.Status = "Admitted";
